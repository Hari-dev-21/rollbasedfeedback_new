...
const UserAccess: React.FC = () => {
  ...
  const handleDirectAccess = () => {
    if (directLinkId.trim()) {
      // Extract form ID from various link formats
      let formId = directLinkId.trim();
      
      // Handle different link formats
      if (formId.includes('/feedback/')) {
        formId = formId.split('/feedback/')[1].replace('/', '');
      } else if (formId.includes('feedback/')) {
        formId = formId.split('feedback/')[1].replace('/', '');
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-8">
          ...
          <p className="text-gray-600 mb-4">
            Have a specific form link? Paste it here to access it directly.
          </p>
          <div className="flex space-x-3">
            <input
              ...
              placeholder="Paste form link or ID here..."
              className="flex-1 border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500"
            />
            <button
              onClick={handleDirectAccess}
              disabled={!directLinkId.trim()}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          <div className="bg-white rounded-lg shadow p-6 text-center">
            ...
            <div className="text-3xl font-bold text-gray-900">{filteredForms.length}</div>
          ...
          <div className="bg-white rounded-lg shadow p-6 text-center">
            ...
            <div className="text-3xl font-bold text-gray-900">
              {filteredForms.reduce((total, form) => total + form.response_count, 0)}
            </div>
          ...
          {loading ? (
            ...
            <div className="bg-white shadow rounded-lg p-12 text-center">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {error 
                  ? 'Unable to load forms at the moment.' 
                  : searchTerm 
        ...
        <div className="bg-blue-50 rounded-lg p-8 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-gray-600">
            <div>
              <h4 className="font-semibold text-gray-900 mb-2">Browse Available Forms</h4>
              <p>View all active feedback forms below and click "Take Survey" to participate.</p>
            </div>
            <div>
              <h4 className="font-semibold text-gray-900 mb-2">Direct Link Access</h4>
              <p>If you have a specific form link, paste it in the "Access Form Directly" section above.</p>
            </div>
            <div>
              <h4 className="font-semibold text-gray-900 mb-2">Search Forms</h4>
              <p>Use the search bar to find specific forms by title, description, or category.</p>
            </div>
            <div>
              <h4 className="font-semibold text-gray-900 mb-2">Form Status</h4>
              <p>Only active, non-expired forms are shown. Inactive forms cannot be accessed.</p>
            </div>
          </div>
        </div>
